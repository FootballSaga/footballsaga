// client/src/TacklingScene.js
import axios from "axios";
const API = "http://localhost:4000";
const TRAINING_DURATION = 60 * 1000; 

export default class TacklingScene extends Phaser.Scene {
  constructor() {
    super("TacklingScene");
    this.playerId = null;
  ...
  async create() {
    ...
    // Background
    this.add.image(width / 2, height / 2, "tackling_bg")
      .setOrigin(0.5)
      .setDisplaySize(width, height);

    this.add.text(width / 2, 100, "TACKLING TRAINING", {
  ...
  async loadTrainingState() {
    try {
      ...
      if (player.training_end) {
        ...
        const startRes = await axios.post(`${API}/players/${this.playerId}/start-training`, { type: "defender_special" });
  ...
  showRewardPopup(player) {
    ...
    if (player.lastStatGain && player.lastStatGain.value > 0) {
      this.add.image(positions.statIcon.x, positions.statIcon.y, "tackling_icon").setDisplaySize(64, 64).setOrigin(0.5).setDepth(102);
