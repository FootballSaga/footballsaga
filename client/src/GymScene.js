...
export default class GymScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("strenght_icon", "/icons/strength_icon.png");
  }
  ...
  showRewardPopup(player) {
    ...
    if (player.lastStatGain && player.lastStatGain.value > 0) {
      this.add.image(positions.statIcon.x, positions.statIcon.y, "strenght_icon").setDisplaySize(64, 64).setOrigin(0.5).setDepth(102);
