// client/src/RunningScene.js
import axios from "axios";
const API = "http://localhost:4000";
const TRAINING_DURATION = 45 * 1000; 
...
export default class RunningScene extends Phaser.Scene {
  ...
  async create() {
    ...
    // Background
    this.add.image(width / 2, height / 2, "running_bg")
      .setOrigin(0.5)
      .setDisplaySize(width, height);

    this.add.text(width / 2, 100, "RUNNING TRAINING", {
  ...
  async loadTrainingState() {
    try {
      ...
      if (player.training_end) {
        ...
        const startRes = await axios.post(`${API}/players/${this.playerId}/start-training`, { type: "running" });
  ...
  showRewardPopup(player) {
    ...
    if (player.lastStatGain && player.lastStatGain.value > 0) {
      this.add.image(positions.statIcon.x, positions.statIcon.y, "speed_icon").setDisplaySize(64, 64).setOrigin(0.5).setDepth(102);
