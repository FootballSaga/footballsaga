...
const TRAINING_DURATION = 60 * 1000; // 30s gym
...
export default class BallScene extends Phaser.Scene {
  ...
  async create() {
    ...
    this.add.text(width / 2, 100, "STAMINA TRAINING", {
  ...
  showRewardPopup(player) {
    ...
    if (player.lastStatGain && player.lastStatGain.value > 0) {
      this.add.image(positions.statIcon.x, positions.statIcon.y, "stamina_icon").setDisplaySize(64, 64).setOrigin(0.5).setDepth(102);
